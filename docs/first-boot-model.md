...
The first-boot model is planned and not implemented. A future appliance must
require a unique local credential setup or an equivalent authorized process;
it must never expose a reusable public default password.
...
No appliance or first-boot automation is included in this baseline.
